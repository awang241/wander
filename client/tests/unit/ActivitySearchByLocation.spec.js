...
describe('ActivitySearchByLocation.vue', () => {
    ...
    test('An activity with a date has their details correctly formatted for the information window', () => {
        ...
        wrapper.vm.formattedStartTime = activity.startTime.slice(8, 10) + "/" + activity.startTime.slice(5, 7) + "/" + activity.startTime.slice(0, 4) +  " at " + activity.startTime.slice(11, 19);
        wrapper.vm.formattedEndTime = activity.endTime.slice(8, 10) + "/" + activity.endTime.slice(5, 7) + "/" + activity.endTime.slice(0, 4) +  " at " + activity.endTime.slice(11, 19);
        ...
        const expectedFormat =
            ...
            `<br>` +
            `<h1 style="font-weight: bold">Start date and time: <span>24/09/2020 at 23:00:00</span></h1>` +
            `<br>` +
            `<h1 style="font-weight: bold">End date and time: <span>26/09/2020 at 11:00:00</span></h1>` +
            `<br>` +
            `<h1 style="font-size: 16px; font-weight: bold"> Activity Types:</h1>` +
            `<span style="color:red; font-weight: bold">*</span><span> ${activity.activityTypes[0]} </span>`+
            `</div>`
    ...
    test('An activity without a date has their details correctly formatted for the information window', () => {
        ...
        const expectedFormat =
            ...
            `<br>` +
            `<h1 style="font-weight: bold">End date: Never! "${activity.activityName}" is continuous!</h1>` +
            `<br>` +
            `<h1 style="font-size: 16px; font-weight: bold"> Activity Types:</h1>` +
            `<span style="color:red; font-weight: bold">*</span><span> ${activity.activityTypes[0]} </span>`+
            `</div>`
